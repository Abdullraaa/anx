...
- **`ProtectedRoute` gates on role, not just session.** A rider can authenticate here — login goes straight to Supabase and knows nothing about roles — so the dashboard turns non-admins away itself: it signs them out and redirects with **"Admin access only"**. `LoginPage`'s own redirect also checks `role === 'admin'`; without that a rejected rider ping-pongs between the two while sign-out resolves. Gate on `loading` before judging the role, or a real admin flashes the rejection during the profile fetch.
...
- **`requireAdmin` on the job collection routes is a security fix — don't remove it.** `GET /api/jobs`, `POST /api/jobs`, `PATCH /api/jobs/:id/assign` and `PATCH /api/jobs/:id/cancel` are all admin-gated. The first two were unguarded until PR #3: because the backend holds the service-role key and bypasses RLS, `GET /api/jobs` returned **every** job — customer names, phones, addresses, fees — to any authenticated rider, and `POST` let them create jobs. Runtime-verified against production: a rider token gets 403 on both, an admin token 200.
- Rider-facing routes are deliberately **auth-only, not admin-gated**, and self-scope to `req.user.id`: `GET /api/rider/jobs`, `PATCH /api/jobs/:id/status`, `PATCH /api/jobs/:id/payment`, `PATCH /api/rider/push-token`. Adding `requireAdmin` to these breaks the rider app.
- SQL lives in `backend/supabase/migrations/` (`001` schema + RLS, `002` seed zones/pricing, `003` storage policies, `004` rider deactivation, `005` deactivation RLS + `is_my_account_active()`, `006` rider column allowlist, `007` interim zone pricing). Migrations here **auto-apply to production on merge to `main`** via the Supabase–Vercel GitHub integration ("Deploy to production" enabled). Don't hand-apply a migration that's already merged — you'll run it twice.
  - **The migration table is not a reliable record of what's applied.** Checked 2026-09-26: production records only three (`005`, `allowlist_rider_job_columns`, `interim_pricing`) because `001`–`004` were applied by hand in the SQL Editor before the integration existed. `anx-dev` records eight, including `007` twice under two names (`interim_pricing`, then `interim_pricing_25_zones`) from being applied before and after Mpape. Neither list is the schema. Verify against `pg_policies` / `pg_proc` / actual row counts instead.
...
Supabase's security advisor flags four items, all `WARN`, none `ERROR`. Re-verified against
production on 2026-09-26 — unchanged after `007` — and **deliberately parked** until after
the pitch. They predate and are unrelated to the role/column fixes in migration `006`. They
apply to **both** Supabase projects (same schema), and the fix belongs in a migration
(`008` — `007` is now the pricing migration) so it flows through the normal path rather
than a console edit.
...
- **The two env stores now point at different Supabase projects.** `rider-app/.env` targets **anx-dev** (local Expo Go), while the variables stored on EAS still target **production** — so an APK built today talks to production while Expo Go talks to anx-dev. That is intentional, but it means a bug reproducible in one may not appear in the other; check which backend and which project you're actually on before concluding anything.
